...
public class CmdTabletopKicklist extends Subcommand {
	...
	public void run(CommandSource s, String[] args) {
		...
		if (sess == null && !(sess instanceof GameInstance)) {
			Util.msg(s, Component.text("You're not in a game instance!", NamedTextColor.RED));
			return;
		}
		
		if (!sess.getHost().equals(uuid)) {
			Util.msg(s, Component.text("Only the host may access the kick list!", NamedTextColor.RED));
			return;
		}
